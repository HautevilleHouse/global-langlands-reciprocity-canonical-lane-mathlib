...
/-!
# Source-derived formalization layer for `global-langlands-reciprocity-canonical-lane`
...
def sourceFormulaModels : List SourceFormulaModel := [
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", expr := (FormulaExpr.var "eps_coh_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Appendix E.6", notes := "Strict coherence target.", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"], components := [
    { key := "eps_coh_raw", value := "0.0" }
  ] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", expr := (FormulaExpr.div (FormulaExpr.num "1.0") (FormulaExpr.add (FormulaExpr.num "1.0") (FormulaExpr.var "delta_comp_sup_raw"))), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Section 5.3 / Appendix C", notes := "kappa_compact theorem-level floor.", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"], components := [
    { key := "delta_comp_sup_raw", value := "0.244" }
  ] },
  { group := "constants", key := "kappa_compatibility", status := "derived_numeric", formula := "c_comp_raw * compatibility_density_raw - e_comp_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_comp_raw") (FormulaExpr.var "compatibility_density_raw")) (FormulaExpr.var "e_comp_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Section 4.5 / Appendix A", notes := "kappa_compatibility theorem-level floor.", validation := "required_positive", componentKeys := ["c_comp_raw", "compatibility_density_raw", "e_comp_raw"], components := [
    { key := "c_comp_raw", value := "1.452" },
    { key := "compatibility_density_raw", value := "0.916" },
    { key := "e_comp_raw", value := "0.236" }
  ] },
  { group := "constants", key := "reciprocity_transfer", status := "derived_numeric", formula := "c_recip_raw * transfer_gain_raw - e_recip_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_recip_raw") (FormulaExpr.var "transfer_gain_raw")) (FormulaExpr.var "e_recip_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Section 6.2 / Appendix E.4", notes := "reciprocity_transfer theorem-level floor.", validation := "required_positive", componentKeys := ["c_recip_raw", "e_recip_raw", "transfer_gain_raw"], components := [
    { key := "c_recip_raw", value := "1.119" },
    { key := "e_recip_raw", value := "0.121" },
    { key := "transfer_gain_raw", value := "1.03" }
  ] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", expr := (FormulaExpr.var "rho_rigidity_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Section 6.1 / Appendix D", notes := "rho_rigidity theorem-level floor.", validation := "required_positive", componentKeys := ["rho_rigidity_raw"], components := [
    { key := "rho_rigidity_raw", value := "1.079" }
  ] },
  { group := "constants", key := "sigma_localglobal", status := "derived_numeric", formula := "localglobal_floor_raw - stabilization_loss_raw - restart_loss_raw", expr := (FormulaExpr.sub (FormulaExpr.sub (FormulaExpr.var "localglobal_floor_raw") (FormulaExpr.var "stabilization_loss_raw")) (FormulaExpr.var "restart_loss_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Section 5.2 / Appendix B", notes := "sigma_localglobal theorem-level floor.", validation := "required_positive", componentKeys := ["localglobal_floor_raw", "restart_loss_raw", "stabilization_loss_raw"], components := [
    { key := "localglobal_floor_raw", value := "1.392" },
    { key := "restart_loss_raw", value := "0.149" },
    { key := "stabilization_loss_raw", value := "0.168" }
  ] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", expr := (FormulaExpr.var "sigma_star_can_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/GLOBAL_LANGLANDS_RECIPROCITY_PREPRINT.md Section 5 / stitch constants", notes := "sigma_star_can theorem-level stitch constant.", validation := "required_positive", componentKeys := ["sigma_star_can_raw"], components := [
    { key := "sigma_star_can_raw", value := "1.054" }
  ] }
]

def formalizationCertificate : FormalizationCertificate := {
  sourceRepo := "global-langlands-reciprocity-canonical-lane",
